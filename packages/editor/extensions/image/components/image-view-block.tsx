...
export const ImageViewBlock: React.FC<NodeViewProps> = ({
  ...
  const [imageDirection, setImageDirection] = React.useState<
    'justify-left' | 'justify-center' | 'justify-end'
  >('justify-center');
  ...
  return (
    <NodeViewWrapper
      ...
      data-drag-handle
      className={cn(
        'relative flex w-full text-center leading-none',
        imageDirection
      )}
    >
      <div
          ...
          {!isResizing &&
            ...
            !imageState.isServerUploading && (
              <ImageActions
                ...
                onLeft={() => setImageDirection('justify-left')}
                onCenter={() => setImageDirection('justify-center')}
                onRight={() => setImageDirection('justify-end')}
              />
            )}
        </div>
      </div>
