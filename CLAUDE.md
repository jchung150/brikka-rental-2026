...
## ⚠️ 진행 중인 방향 전환 — 먼저 읽을 것

**이 시스템은 주거용 → 상업용 빌딩 임대관리로 전환 중입니다.** 아래 문서를 먼저 읽으세요:

### 👉 `docs/작업_인수인계.md`

- 외주 개발사는 계약 종료로 **존재하지 않습니다.** 발주처 대표(**비개발자**)가 직접 인수해 진행 중입니다.
- 현재 작업: 기존 기능정의서(엑셀 72행)를 **한 행씩 검토하며 상업용으로 개정**. **코드/스키마는 아직 전혀 손대지 않았습니다.**
- 오픈 범위: **관리자(백오피스) 기능만 먼저.** 임대인/입주자 포털은 2차.
- 전역 결정(차량번호 미관리, 용어 `임차인` 통일, 가전·가구 삭제 등)과 누적 스키마 변경 목록이 인수인계 문서에 있습니다.

아래 §1~§5는 **인수 시점의 코드 감사 결과**이며 여전히 유효합니다(코드는 그대로이므로). 다만 §3의 "주거용 전제"는 위 전환 방침이 우선합니다.
